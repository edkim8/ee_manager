-- Fix b_b format to show clean integers (2x2 instead of 2.0x2.0)
-- Date: 2026-02-16
...
SELECT
  ...
  fp.bedroom_count,
  fp.bathroom_count,
  -- Bedroom x Bathroom concatenation (FIXED: Cast to INTEGER for clean display)
  CASE
...
SELECT
  ...
  fp.bedroom_count,
  fp.bathroom_count,
  -- Bedroom x Bathroom concatenation (FIXED: Cast to INTEGER)
  CASE
    WHEN fp.bedroom_count IS NOT NULL AND fp.bathroom_count IS NOT NULL THEN
      CONCAT(fp.bedroom_count::INTEGER, 'x', fp.bathroom_count::INTEGER)
    ELSE NULL
...
COMMENT ON VIEW view_leasing_pipeline IS 'Hybrid leasing pipeline with pricing, concessions (C%/A%), turnover metrics, and clean b_b format (2x2).';
