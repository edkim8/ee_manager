...
function generateManagerBriefHtml(brief: string, date: string): string {
  ...
  function renderTable(tableLines: string[]): string {
    ...
    const headerHtml = headers.map(h => `<th>${applyInline(h)}</th>`).join('')
    const bodyHtml = bodyLines
      ...
      .join('\n')
    return `<table class="brief-table"><thead><tr>${headerHtml}</tr></thead><tbody>${bodyHtml}</tbody></table>`
  }
...
<style>
  ...
  .header { background: #b45309; color: #ffffff; padding: 20px 28px; }
  .header h1 { margin: 0; font-size: 18px; font-weight: 600; }
  .header p { margin: 4px 0 0; font-size: 12px; color: #fde68a; }
  .draft-banner { background: #fef3c7; border-bottom: 2px solid #f59e0b; padding: 10px 28px; font-size: 12px; color: #92400e; font-weight: 600; }
  .body { padding: 20px 28px; }
  h2 { font-size: 15px; color: #b45309; border-bottom: 1px solid #fde68a; padding-bottom: 4px; margin: 18px 0 8px; }
  h3 { font-size: 13px; color: #374151; margin: 12px 0 4px; }
  hr { border: none; border-top: 1px solid #e2e8f0; margin: 14px 0; }
  p { font-size: 13px; line-height: 1.6; color: #374151; }
  strong { color: #111827; }
  code { font-family: monospace; font-size: 12px; background: #f1f5f9; padding: 1px 4px; border-radius: 3px; }
  .brief-table { width: 100%; border-collapse: collapse; margin: 10px 0 16px; font-size: 12px; }
  .brief-table thead tr { background: #b45309; }
  .brief-table th { color: #ffffff; padding: 7px 10px; text-align: left; font-weight: 600; font-size: 11px; text-transform: uppercase; letter-spacing: 0.04em; }
  .brief-table td { padding: 6px 10px; border-bottom: 1px solid #e2e8f0; color: #374151; vertical-align: top; }
  .brief-table tbody tr:last-child td { border-bottom: none; }
  .footer { padding: 14px 28px; background: #f8f9fa; border-top: 1px solid #e2e8f0; font-size: 11px; color: #9ca3af; }
</style>
</head>
<body>
<div class="container">
  <div class="header">
    <h1>Property Operations Brief — ${date}</h1>
    <p>Action items requiring manager attention</p>
  </div>
  <div class="draft-banner">⚠️ DRAFT — Review and edit before forwarding to property managers</div>
  <div class="body">
    ${parts.join('\n')}
  </div>
  <div class="footer">
    Generated by EE Manager &nbsp;|&nbsp; Daily Solver Audit System
  </div>
</div>
</body>
</html>`
...
export default defineEventHandler(async (event) => {
  ...
  const { content, date, batchId, managerBrief } = body
  ...
  try {
    ...
    // If a manager brief was provided, send it as a separate draft email to audit
    // recipients only (not managers) — reviewer edits and forwards manually.
    if (managerBrief) {
      const briefHtml = generateManagerBriefHtml(managerBrief, date)
      for (const email of uniqueEmails) {
        try {
          await transporter.sendMail({
            from: `"${config.public.mailersendSmtpName || 'EE Manager'}" <${config.public.mailersendUsername || 'noreply@ee-manager.com'}>`,
            to: email,
            subject: `[DRAFT] Manager Brief — ${date}`,
            html: briefHtml
          })
          results.push({ email, status: 'brief-sent' })
        } catch (sendError: any) {
          console.error(`[Manager Brief] Failed to send to ${email}:`, sendError)
          results.push({ email, status: 'brief-failed', error: sendError.message })
        }
      }
    }
